...
from key_maze_metrics_extensions import RepresentationQualityMetrics
...
class KeyMazeMetricsTracker:
    ...
    def log_training_step(self, state, action, reward, next_state, done, info, step_count, episode_step):
        ...
        # Log representation metrics (this is the only metric type that is handled separately)
    ...
    def save_metrics(self):
        ...
        try:
            ...
            # Include representation metrics if available
            if hasattr(self, 'representation_metrics'):
                metrics_to_save['representation_metrics'] = self.convert_numpy_to_python(self.representation_metrics)
    ...
    def generate_plots(self):
        ...
        # Define final suffix
        suffix = "_final"  # This is the format other methods expect
        
        # Generate plots
        self._generate_performance_plots(self.dirs['plots'], suffix)
        self._generate_exploration_plots(self.dirs['plots'], suffix)
        self._generate_option_quality_plots(self.dirs['plots'], suffix)
        
        # Force an evaluation of representation quality before plotting
        if hasattr(self, 'rep_extension'):
            print("\n[DEBUG] Forcing evaluation of representation quality metrics")
            # Get the current step count
            step_count = self.performance_metrics['total_env_steps']
            # Create representation directory if not exists
            if 'representation' not in self.dirs:
                self.dirs['representation'] = os.path.join(self.results_dir, "representation_metrics")
                os.makedirs(self.dirs['representation'], exist_ok=True)
            
            # Explicitly evaluate representation quality
            self.rep_extension._compute_true_laplacian_eigenvectors()
            self.rep_extension._evaluate_eigenvector_alignment()
            
            # Now try to plot
            self.rep_extension.plot_representation_quality(final=True)
    ...
    def _generate_exploration_plots(self, plots_dir, suffix):
        ...
        if 'step_coverage' in self.exploration_metrics and self.exploration_metrics['step_coverage']:
            iterations, coverages = zip(*self.exploration_metrics['step_coverage'])
            plt.figure(figsize=(10, 6))
        ...
        if 'state_visitation' in self.exploration_metrics and self.exploration_metrics['state_visitation']:
            # Create a 2D grid for the heatmap
            maze_size = self.env.unwrapped.maze_size
            heatmap = np.zeros((maze_size, maze_size))
        ...
        key_steps = self.exploration_metrics.get('keys_collected_steps', [])
        door_steps = self.exploration_metrics.get('doors_opened_steps', [])
        goal_steps = self.exploration_metrics.get('goal_reached_steps', [])
    ...
    def _generate_option_quality_plots(self, plots_dir, suffix):
        ...
        if 'option_selection_counts' in self.option_metrics and self.option_metrics['option_selection_counts']:
            # Convert to list for plotting
            option_ids = sorted(self.option_metrics['option_selection_counts'].keys())
            usage_counts = [self.option_metrics['option_selection_counts'][i] for i in option_ids]
        ...
        if 'option_durations' in self.option_metrics and any(self.option_metrics['option_durations'].values()):
            plt.figure(figsize=(12, 6))
        ...
        if 'option_rewards' in self.option_metrics and self.option_metrics['option_rewards'] and any(self.option_metrics['option_rewards'].values()):
            plt.figure(figsize=(12, 6))
        ...
        if hasattr(self, 'option_time_metrics') and 'steps' in self.option_time_metrics and self.option_time_metrics['steps']:
        ...
        for opt_id in range(self.agent.num_options):
            if ('option_state_visitation' in self.option_metrics and
                opt_id in self.option_metrics['option_state_visitation'] and
                self.option_metrics['option_state_visitation'][opt_id]):
                # Create a 2D grid for the heatmap
                heatmap = np.zeros((maze_size, maze_size))
    ...
    def _generate_performance_plots(self, plots_dir, suffix):
        ...
        if 'evaluation_rewards' in self.performance_metrics and self.performance_metrics['evaluation_rewards'] and \
           'evaluation_success_rates' in self.performance_metrics and self.performance_metrics['evaluation_success_rates']:
