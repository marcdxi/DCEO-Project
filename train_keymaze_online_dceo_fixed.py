...
def train_agent(config, num_iterations=100, eval_frequency=10, num_train_steps=10000, render_training=False):
    ...
    env = KeyDoorMazeEnv(
        ...
        max_steps=100  # Fixed at 100 steps instead of maze_size * 4
    )
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        if render_training:
            ...
            if maze_img is not None:
                ...
                title = f"Training - Iteration {iteration}/{num_iterations}, New Episode {episodes_completed+1} (Total Steps: {total_training_steps})\n"
        ...
        for step in range(1, num_train_steps + 1):
            ...
            # Visualize every few steps or on important events
            if render_training and (step % 5 == 0 or info.get('key_collected', False) or 
                ...
                if maze_img is not None:
                    ...
                    title += f"Episode: {episodes_completed+1}, Reward: {episode_reward:.2f}\n"
                    ...
                    if current_option is not None:
                        ...
                        title += f"Using main policy, Option Changes: {option_changes}"
    ...
    # Final evaluation
    print("\nFinal Evaluation")
    avg_reward, success_rate, _, _, _ = evaluate_agent(
        agent, env, num_episodes=10, fig=fig, axes=axes, plot_metrics=plot_metrics
    )
